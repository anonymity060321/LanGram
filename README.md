...
当前阶段重点是稳定核心聊天体验、文件与图片收发、好友系统、群聊基础能力，以及桌面端使用体验。
...
- `.env` 不提交，真实密钥、SMTP 密码、数据库密码和 token 不应进入仓库。
...
LanGram 当前仍处于 MVP 开发阶段，已具备基础账号、好友、单聊、群聊、图片/文件收发和桌面端能力，但不应被视为生产级安全通信软件。当前消息内容加密是 MVP 级能力，不等同于完整 E2EE 或 Secret Chat。
